...
impl TopicListView {
    ...
    fn render_changed_parts(
        ...
        // Calculate available height
        let available_height = (end_row - start_row) as usize;
        let list_start_row = start_row + 2; // Header + separator
        let available_list_height = available_height.saturating_sub(2);
        
        // Check if we need to redraw header (usually not needed unless terminal resize)
        // For now, always redraw header to be safe
        stdout.queue(cursor::MoveTo(0, start_row))?;
        stdout.queue(SetForegroundColor(Color::Cyan))?;
        let header = format!(
            "{:<12} │ {:<18} │ {:<6} │ Latest Payload",
            "Last Message", "Topic", "Count"
        );
        stdout.queue(Print(&header))?;
        stdout.queue(ResetColor)?;
        
        // Redraw separator
        let content_start_row = start_row + 1;
        stdout.queue(cursor::MoveTo(0, content_start_row))?;
        let separator = "─".repeat(terminal_width.min(80) as usize);
        stdout.queue(Print(&separator))?;
        
        // Handle empty state vs non-empty state transitions
        let was_empty = prev_state.topics.is_empty();
        let is_empty = state.topics.is_empty();
        
        if is_empty && was_empty {
            // Both empty, no need to redraw the "no topics" message
        } else if is_empty && !was_empty {
            // Transitioned from having topics to empty, clear all and show message
            for i in list_start_row..(list_start_row + available_list_height as u16) {
                stdout.queue(cursor::MoveTo(0, i))?;
                stdout.queue(Clear(ClearType::CurrentLine))?;
            }
            
            let row = list_start_row + (available_list_height / 2) as u16;
            stdout.queue(cursor::MoveTo(0, row))?;
            stdout.queue(SetForegroundColor(Color::DarkGrey))?;
            let message = "No MQTT topics available. Waiting for messages...";
            let centered_x = (terminal_width.saturating_sub(message.len() as u16)) / 2;
            stdout.queue(cursor::MoveTo(centered_x, row))?;
            stdout.queue(Print(message))?;
            stdout.queue(ResetColor)?;
        } else if !is_empty && was_empty {
            // Transitioned from empty to having topics, clear message area first
            for i in list_start_row..(list_start_row + available_list_height as u16) {
                stdout.queue(cursor::MoveTo(0, i))?;
                stdout.queue(Clear(ClearType::CurrentLine))?;
            }
        ...
        if !is_empty {
            // Compare and update only changed topic rows
            let prev_visible_topics: std::collections::HashMap<usize, &TopicStat> = 
                (prev_state.scroll_offset..)
                    .enumerate()
                    .take(available_list_height)
                    .filter_map(|(display_idx, topic_idx)| {
                        prev_state.topics.get(topic_idx).map(|topic| (display_idx, topic))
                    })
                    .collect();
            ...
            for (display_index, topic_index) in (state.scroll_offset..).enumerate() {
                ...
                let row = list_start_row + display_index as u16;
                let should_redraw = if let Some(current_topic) = state.topics.get(topic_index) {
                    // Check if this row changed
                    let topic_changed = prev_visible_topics.get(&display_index)
                        .map_or(true, |prev_topic| {
                            prev_topic.topic != current_topic.topic ||
                    ...
                    let selection_changed = (prev_state.selected_index - prev_state.scroll_offset == display_index) != 
                                           (state.selected_index - state.scroll_offset == display_index);
                    
                    topic_changed || selection_changed
                } else {
                    // No topic at this position, clear the line
                    true
                };
                ...
                if should_redraw {
                    ...
                    if let Some(topic) = state.topics.get(topic_index) {
                        ...
                        Self::render_topic_row(&mut stdout, topic, terminal_width)?;
        ...
        let current_visible_count = (state.topics.len() - state.scroll_offset).min(available_list_height);
        let prev_visible_count = (prev_state.topics.len() - prev_state.scroll_offset).min(available_list_height);
        
        if prev_visible_count > current_visible_count {
            for i in (list_start_row + current_visible_count as u16)..(list_start_row + prev_visible_count as u16) {
                stdout.queue(cursor::MoveTo(0, i))?;
                stdout.queue(Clear(ClearType::CurrentLine))?;
            }
        }
